@extends('layouts.master') @section('title','Update Role')
...
@section('content')
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li><a href="{{{url('roles/list')}}}">Role List</a></li>
    ...
    <section class="content">
        ...
        <div class="container-fluid">
            <div class="">
                <div class="col-md-10">
                        <form role="form" action="{{url('roles/update')}}" class="form-horizontal" method="post">
                            {{ csrf_field() }}

                        <div class="row">
                            <div class="form-group col-md-4 m-b-5">
                                <label class="control-label required">Role Name</label>
                                <input type="text" class="form-control @if($errors->has('role_name')) error @endif"  id="role_name" name="role_name" placeholder="Role Name"  value="{{ $roles->name }}">
                                @if($errors->has('role_name'))
                                    <label id="label-error" style="color: red" class="error" for="user_name">{{$errors->first('role_name')}}</label>
                                @endif
                                <input type="hidden" name="id_txt" name="id_txt" value="{{ $roles->id }}">
                            </div>
                        </div>
                            ...
                            <div class="row">
                                <div class="form-group col-md-4 m-b-5">
                                    ...
                                    {!! Form::select('permission[]',$all_access,$access,['class'=>'form-control','style'=>'width:100%;','data-placeholder'=>'Choose Menu','multiple'=>'multiple','id'=>'permission']) !!}
